      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--color-texto-suave)' }}>
            {instancias.length === 0
              ? 'Nenhuma instância do Roblox em execução'
              : `${instancias.length} instância${instancias.length > 1 ? 's' : ''} em execução`}
          </p>
        </div>
                  ...
                  <div className="space-y-1 max-h-60 overflow-y-auto custom-scrollbar">
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-bold text-white/80">Entrar como Convidado</p>
                        <p className="text-[9px] text-white/20">Sem login automático</p>
                      </div>
                    </button>
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                  ...
                  >
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-lg font-bold text-white">Escolha o Cliente</h3>
                      <button 
                    ...
                    <p className="mt-6 text-[10px] text-center text-white/20">
                      Você pode cadastrar mais clientes na engrenagem de configuração.
                    </p>
                  </motion.div>
                </div>
              )}
            ...
            <div className="p-4 rounded-2xl glass border border-white/5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                {caminhoCustomizado && (
                  ...
                  >
                    <X className="w-3 h-3" /> Resetar para Automático
                  </button>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest pl-1">
                    Cadastrar Novo Executável
                  </label>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      placeholder="Ex: Roblox Farming, Main Client..."
                    ...
                    <button
                      onClick={async () => {
                        if (!nomeClienteNovo.trim()) {
                          exibirFeedback('erro', 'Dê um nome para o executável.');
                          return;
                        }
                        const sucesso = await adicionarClienteCustomizado(nomeClienteNovo);
                        if (sucesso) {
                          definirNomeClienteNovo('');
                          exibirFeedback('sucesso', 'Executável cadastrado!');
                        }
                      }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-white truncate">
                            {cliente.tipo === 'Oficial' ? 'Roblox Padrão' : cliente.nome}
                          </p>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            title="Ocultar deste visualizador"
                          >
                    ...
                    {clientesEncontrados.length === 0 && clientesCustomizados.length === 0 && (
                      <div className="col-span-full p-6 text-center border border-dashed border-white/10 rounded-2xl">
                        <p className="text-xs text-white/20 font-medium">Nenhum executável configurado</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {caminhoCustomizado && (
                <div className="mt-2 p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-2">
                  ...
                  <p className="text-[10px] text-emerald-300">
                    Usando caminho customizado: <span className="font-mono opacity-60">{caminhoCustomizado}</span>
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 space-y-3">
              <div>
                ...
                <p className="text-sm text-rose-400/80 leading-relaxed">
                  Não conseguimos localizar automaticamente uma instalação do Roblox, Bloxstrap ou Voidstrap no seu computador.
                </p>
              </div>
          ...
          <div className="text-center space-y-4 max-w-xs">
            ...
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: 'var(--color-texto-suave)' }}>
                Clique em <strong className="text-[var(--color-primaria)]">Nova Instância</strong> para abrir o Roblox
              </p>
            </div>
          </div>
        </motion.div>
      ...
      {instancias.length > 0 && (
        <div className="mt-4 pt-3 border-t" style={{ borderColor: 'var(--color-borda)' }}>
          <p className="text-xs text-center" style={{ color: 'var(--color-texto-apagado)' }}>
            Lista atualizada automaticamente a cada 3 segundos
          </p>
        </div>
      )}
    </div>
