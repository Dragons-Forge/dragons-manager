...
export function SecaoJogos() {
  ...
  const alternarFavorito = async (e: React.MouseEvent, jogo: InfoJogoRoblox) => {
    ...
    try {
      ...
      console.error('Falha ao salvar favoritos no backend', error);
    }
  };
    ...
    <div className="flex flex-col h-full space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Busque, favorite e inicie instâncias do Roblox rapidamente.</p>
        </div>
        ...
        <div className="flex p-1 bg-black/20 rounded-xl border border-white/5 max-w-sm shrink-0">
          <button
            ...
            className={`flex items-center justify-center gap-2 px-6 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all cursor-pointer ${
              abaAtual === 'descobrir' ? 'bg-[var(--cor-primaria)] text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 px-6 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all cursor-pointer ${
              abaAtual === 'favoritos' ? 'bg-rose-500 text-white shadow-lg shadow-rose-500/20' : 'text-gray-500 hover:text-rose-400'
            }`}
          >
        ...
        <form onSubmit={aoSubmeterBusca} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Minecraft, Adopt Me, Blox Fruits..."
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-10">
        {estaCarregando ? (
          <div className="flex flex-col items-center justify-center h-40 space-y-4">
            ...
            <p className="text-sm text-gray-400">Buscando as melhores experiências...</p>
          </div>
        ) : erroMensagem ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 space-y-2 text-gray-500">
            {abaAtual === 'favoritos' ? (
              <>
                ...
                <p className="text-xs">Vá para a aba "Descobrir" e marque com o coração.</p>
              </>
            ) : (
              <>
        ...
        {abaAtual === 'descobrir' && cursorProximo && !estaCarregando && (
          <div className="flex justify-center mt-8">
            ...
            >
              {estaCarregandoMais ? <Loader2 className="w-4 h-4 animar-girar" /> : 'Carregar mais...'}
            </button>
          </div>
        )}
      </div>
